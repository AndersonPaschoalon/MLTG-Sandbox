...
class TracePlotter:
    ...
    def __init__(self, plot_color, trace_id, flow_database_file):
        self.plot_color = plot_color
        self.trace_id = trace_id
        self.flow_database_file = flow_database_file

    def fetch_packets(self):
        conn = sqlite3.connect(self.flow_database_file)
        c = conn.cursor()
        c.execute("SELECT tsSec, tsUsec COUNT(*) FROM Packets WHERE traceID=? ORDER BY tsSec, tsUsec", (self.trace_id,))
        rows = c.fetchall()
        conn.close()
        return rows
