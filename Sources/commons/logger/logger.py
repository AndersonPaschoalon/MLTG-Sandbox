...
class Logger:
    ...
    def initialize(log_file, level_log=logging.DEBUG, level_console=logging.WARNING):
        ...
        if Logger._logger:
            Logger._logger.warning(
                f"Logger {Logger.LOGGER_NAME} already initialized. New log file {log_file} will not be created."
            )
            return
        _log_file = log_file
        ...
        # Ensure the file is writable/readable
        log_path.touch(mode=0o666, exist_ok=True)
        os.system(f"chmod 666 {log_path}")
        ...
        if Logger._rotating_method == 1:
            fh = TimedRotatingFileHandler(
                log_file, when="midnight", backupCount=Logger._backup_count
            )
    ...
    def get():
        ...
        ), "Error: get() was called before Logger was initialized. Call Logger.initialize() to use the logger."
